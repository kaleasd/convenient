...
public abstract class AbstractTask implements ITask {
    ...
    protected String getTypeDesc2(Order order) {
        ...
        String s = "";
        if (serviceType == OrderTypeEnum.AIRPORT_PICKUP.getCode()) {
            s = "接机";
    ...
    public JSONArray getTagsJson(String useFeature) {
        ...
        for (String s : ss) {
            ...
            TagInfo tagInfo = DispatchServiceImpl.ins().getTagInfo(id);
            if (tagInfo == null) {
                continue;
            }
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("tagImg", DispatchServiceImpl.ins().getOssFileUrl() + tagInfo.getTagImg());
