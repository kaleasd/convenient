...
public class OrderTask implements ITask {
    ...
    public int execute(long current) {
        ...
        Order order = DispatchServiceImpl.ins().getOrderById(orderId);
        OrderRulePrice orderRulePrice = DispatchServiceImpl.ins().getOrderRulePrice(orderId);
    ...
    public void taskEnd(Order order, OrderRulePrice orderRulePrice) {
        if (type == OrderTypeEnum.NORMAL.getCode()) {
            if (DispatchServiceImpl.ins().hasDriver(orderRulePrice.getCityCode(), order.getOrderStartTime(), orderRulePrice.getCarLevelId(), order.getServiceType())) {
                ...
                for (DriverData data : list) {
                    ...
                    try {
                        ...
                        if (success) {
                            ...
                            DispatchServiceImpl.ins().sendSmsMessage(phone, Const.SMS_FORCE_DISPATCH_DRIVER, smsMap);
                            return;
                        }
                    } catch (Exception e) {
