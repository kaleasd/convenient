...
public class OrderForceTask extends AbstractTask{
    ...
    public boolean sendOrder(Order order, OrderRulePrice orderRulePrice, TaskCondition taskCondition, int round) {
        ...
        try {
            ...
            Order newOrder = DispatchServiceImpl.ins().getOrderById(order.getId());
            if (newOrder.getStatus() != Const.ORDER_STATUS_ORDER_START) {
                return false;
            }
            for (Integer distance : taskCondition.getDistanceList()) {
                List<DriverData> list = DispatchServiceImpl.ins().getCarByOrder(order, taskCondition, distance, usedDriverId, round, true);
                ...
                for (DriverData data : list) {
                    ...
                    try {
                        ...
                        int count = DispatchServiceImpl.ins().countDriverOrder(data.getDriverInfo().getId(), startTime, endTime);
                        if (count > 0) {
                            continue;
                        }
                        String otherPhone = order.getPassengerPhone();
                        if (StringUtils.isNoneEmpty(order.getOtherPhone())) {
                        ...
                        try {
                            if (StringUtils.isNoneEmpty(order.getPassengerPhone())) {
                                bindAxbResponse1 = DispatchServiceImpl.ins().bindAxb(EncriptUtil.decryptionPhoneNumber(data.getDriverInfo().getPhoneNumber()), EncriptUtil.decryptionPhoneNumber(order.getPassengerPhone()), order.getOrderStartTime().getTime() + TimeUnit.DAYS.toMillis(1));
                            ...
                            if (StringUtils.isNotEmpty(order.getOtherPhone())) {
                                bindAxbResponse2 = DispatchServiceImpl.ins().bindAxb(EncriptUtil.decryptionPhoneNumber(data.getDriverInfo().getPhoneNumber()), EncriptUtil.decryptionPhoneNumber(order.getOtherPhone()), order.getOrderStartTime().getTime() + TimeUnit.DAYS.toMillis(1));
                        ...
                        // 派单
                        boolean success = DispatchServiceImpl.ins().updateOrder(updateOrder);
                        if (success) {
                            // 更新高德
                            ...
                            DispatchServiceImpl.ins().updateAmapOrder(orderRequest);
                            status = STATUS_END;
                            ...
                            if (bindAxbResponse2 != null) {
                                DispatchServiceImpl.ins().unbind(bindAxbResponse2.getAxbSubsId(), bindAxbResponse2.getAxbSecretNo());
                            }
                            if (bindAxbResponse1 != null) {
                                DispatchServiceImpl.ins().unbind(bindAxbResponse1.getAxbSubsId(), bindAxbResponse1.getAxbSecretNo());
                            }
                        }
                    } catch (Exception e) {
    ...
    private void pushPassenger(Order order, OrderRulePrice orderRulePrice, DriverData driverData, String driverPhone) {
        try {
            ...
            DispatchServiceImpl.ins().sendSmsMessageHx(EncriptUtil.decryptionPhoneNumber(order.getPassengerPhone()), Const.HX_FORCE_DISPATCH_PASSENGER2, name, plateNumber, driverPhone, servicePhone);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private void pushOther(Order order, OrderRulePrice orderRulePrice, DriverData driverData, String driverPhone) {
        try {

            if (order.getOrderType() != 2) {
                return;
            }
            PassengerInfo passengerInfo = DispatchServiceImpl.ins().getPassengerInfo(order.getPassengerInfoId());
            String timeDesc = DateUtils.formatDate(order.getOrderStartTime(), DateUtils.yyMMddHHmm);
            ...
            DispatchServiceImpl.ins().sendSmsMessageHx(phone, Const.HX_FORCE_DISPATCH_PASSENGER, pname, color, plateNumber, StringUtils.substring(driverData.getDriverInfo().getDriverName(), 0, 1) + "师傅", driverPhone);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private void smsDriver(Order order, OrderRulePrice orderRulePrice, DriverData data, String passengerPhone, String driverPhone) {
        try {
    ...
    private void smsDriverHx(Order order, OrderRulePrice orderRulePrice, DriverData data, String passengerPhone, String driverPhone) {
        if (order.getServiceType() == OrderTypeEnum.CHARTERED_CAR_FULL.getCode() || order.getServiceType() == OrderTypeEnum.CHARTERED_CAR_HALF.getCode()) {
            DispatchServiceImpl.ins().sendSmsMessageHx(driverPhone, Const.HX_FORCE_DISPATCH_DRIVER_BAOCHE, getTypeDesc(orderRulePrice.getServiceTypeId(), data.getIsFollowing()), DateUtils.formatDate(order.getOrderStartTime(), DateUtils.yyMMddHHmm), order.getStartAddress(), StringUtils.substring(passengerPhone, passengerPhone.length() - 4));
        } else {
            DispatchServiceImpl.ins().sendSmsMessageHx(driverPhone, Const.HX_FORCE_DISPATCH_DRIVER, getTypeDesc(orderRulePrice.getServiceTypeId(), data.getIsFollowing()), DateUtils.formatDate(order.getOrderStartTime(), DateUtils.yyMMddHHmm), StringUtils.substring(passengerPhone, passengerPhone.length() - 4), order.getStartAddress(), order.getEndAddress());
        }
    }

    private void pushPassanger(Order order, OrderRulePrice orderRulePrice, DriverData data, String driverPhone) {
        ...
        try {
            ...
            msg.put("avgGrade", DispatchServiceImpl.ins().getDriverEvaluateByDriverId(data.getDriverInfo().getId()));
    ...
    private void pushDriver(Order order, OrderRulePrice orderRulePrice, DriverData data, String passengerPhone) {
        try {
            String timeDesc = DateUtils.formatDate(order.getOrderStartTime(), DateUtils.yyMMddHHmm);
            JSONObject msg = new JSONObject();
            ...
            double startAddressDistance = DispatchServiceImpl.ins().calDistance(data.getAmapVehicle().getLongitude(), data.getAmapVehicle().getLatitude(), order.getStartLongitude(), order.getStartLatitude());
            ...
            msg.put("charterCarInfo", DispatchServiceImpl.ins().getChargeRuleStr(chargeRule));
