package com.gyh.servicejmsconsumer.config;
...
public class ActiveMQConfig {
    ...
    public JmsListenerContainerFactory<?> jmsListenerContainerQueue(ActiveMQConnectionFactory connectionFactory) {
        ...
        /**
         * 客户端手动确认，这就意味着ActiveMQ将不会自动ACK任何消息
         * 如果一个consumer在消费结束前没有调用message。acknowledge()确认一个消息，
         * 之后调用其他consumer时会再次消费它，因为对于broker而言，那些尚未真正ACK的消息被视为未消费
         * 直到他被确认
         * */
    ...
    public RedeliveryPolicy redeliveryPolicy() {
        ...
        //第一次失败后重新发送之前等待500毫秒，第二次失败在等待500*2毫秒，这里的2就是value
        redeliveryPolicy.setBackOffMultiplier(2);
        //是否避免消息碰撞
        redeliveryPolicy.setUseCollisionAvoidance(false);
        //设置重发最大延时时间-1，表示没有延时只有UseExponentialBackOff(true)为true时生效
